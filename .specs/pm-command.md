...
Running `cuggino` with no subcommand starts an interactive Claude session in **project manager (PM) mode**. This spawns `claude` with an appended system prompt that constrains the agent to act as a project manager — reading specs, discussing features with the user, writing specifications, managing the backlog, and resolving spec issues. The PM does not write code.
...
When `cuggino` is invoked without a subcommand, the root command handler calls `LlmAgent.interactive()` to start an interactive agent session with a system prompt that injects PM constraints. The user interacts with the agent directly in their terminal — `cuggino` simply sets up the session and exits once the agent finishes.
...
```typescript
interface PmCommandPromptOptions {
...
The PM mode is the default handler of the root `cuggino` command (runs when no subcommand is provided):
...
2. Call `LlmAgent.interactive()` with:
   ...
   - `systemPrompt`: the PM system prompt
   - `dangerouslySkipPermissions`: `true`
3. Exit with the exit code returned by `interactive()`

The `LlmAgent` implementation (e.g., `ClaudeLlmAgent`) is responsible for spawning the underlying process with inherited stdio. The root command handler does not interact with child processes directly.
