...
import { ClaudeLlmAgentLayer } from "../../src/ClaudeLlmAgent.js"
import { extractMarkers } from "../../src/extractMarkers.js"
import { defaultMarkerConfig } from "../../src/LlmMarkerEvent.js"
import { NodeServices } from "@effect/platform-node"

describe("LlmAgent E2E", () => {
  it("should spawn Claude and receive events", async () => {
    const program = Effect.gen(function*() {
      ...
      // Find the SystemMessage event
      const systemMessage = collected.find((e) => e._tag === "SystemMessage")
    ...
    const result = await Effect.runPromise(
      program.pipe(
        Effect.provide(ClaudeLlmAgentLayer.pipe(Layer.provide(NodeServices.layer) )),
        Effect.scoped
      )
    )
  ...
  it("should extract markers from Claude output", async () => {
    const program = Effect.gen(function*() {
      const agent = yield* LlmAgent
